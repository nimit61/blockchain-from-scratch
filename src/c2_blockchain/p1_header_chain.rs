...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for block in chain {
            if block.parent != hash(current_block) || block.height != current_block.height + 1 {
                return false;
            }
            current_block = block;
        }
...
fn build_valid_chain_length_5() -> Vec<Header> {
    ...
    let mut current_block = g;

    let mut chain: Vec<Header> = Vec::new();

    for i in 0..4 {

        let next_block = current_block.child();
...
fn bc_1_invalid_chain_is_really_invalid() {
    // This test chooses to use the student's own verify function.
    // This should be relatively safe given that we have already tested that function.
    let invalid_chain: Vec<Header> = build_an_invalid_chain();
